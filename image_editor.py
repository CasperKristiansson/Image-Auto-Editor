...
def add_text(image, text, font, text_color, text_start_height, width):
...
def add_text(background, text, width):
    ...
    add_text(background, text, font, text_color, text_start_height, width)

    return background

def image_editor(backgrounds, logo_image, backgrounds_name, quote_list):
...
if __name__ == '__main__':
    ...
    LOGO = get_logo(START_DIRECTORY)
    QUOTE_LIST = get_list()
    image_editor(BACKGROUNDS, LOGO, BACKGROUNDS_NAME, QUOTE_LIST)
